...
- ~~**Mapping-editor right panel search filter renders empty results.**~~ Fixed on `fix/show-search-results-and-bun-test`. Root cause was not the filter (which was always correct): `Keymap.dispatch()` had no notion of a focused text input, so typing "?" while searching matched the global help binding and opened the overlay, which then swallowed every further keystroke via `stopPropagation()`, freezing the results list. Added `KeymapOptions.textInputActive` (`src/tui/utils/keymap.ts`) to suppress printable-key bindings while a screen reports a text input focused, wired up on mapping-editor's search box, mapping-save's name/version fields, and settings' inline field editor (all three had the same latent flaw).
- ~~**`bun test` hangs when `tests/tui/screens/mapping-editor.test.ts` is targeted in isolation**~~ Not a bug: `bunfig.toml` (added in TR.A4) scopes `bun test` to `root = "tests/lib"` precisely because TUI tests are vitest-authored (`vi.mock`, `importOriginal`) and Bun's runner has no equivalent. Passing an explicit path argument overrides `root`, so Bun loads a vitest file anyway and hangs rather than erroring, since the bare `vitest` package never receives the worker handshake it expects. The original "296 pass / 0 fail" reading of `bun test` was `tests/lib` only, never the full suite; `test:all` is the full suite. Added `bun run test:tui` (`vitest run tests/tui`) as the correct single-runner invocation, documented in `scripts/README.md`.
...
- Update/extend the relevant `tests/tui/*` suite; `bun run test:tui` green (`tests/tui/**` is vitest-authored, not `bun test` — see Known Issues above and `scripts/README.md`).
