...
describe('checkWorkflow', () => {
	...
	describe('anomaly detection', () => {
		it('detects significant learner count increase', async () => {
			...
			// Create the previous submission file so filesystem check passes
			const previousFile = join(testDir, 'ILR-2026-01-15T10-00-00.xml');
			await writeFile(previousFile, fixtures.validXmlTwoLearners);
		...
		it('detects schema version changes', async () => {
			...
			// Create the previous submission file so filesystem check passes
			const previousFile = join(testDir, 'ILR-2025-09-01T10-00-00.xml');
			await writeFile(previousFile, fixtures.validXmlTwoLearners);
		...
		it('handles zero learner count in previous submission', async () => {
			...
			// Create the previous submission file so filesystem check passes
			const previousFile = join(testDir, 'ILR-2026-01-01.xml');
			await writeFile(previousFile, fixtures.validXmlTwoLearners);
