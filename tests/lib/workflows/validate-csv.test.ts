...
import type { WorkflowResult, WorkflowStepEvent, ValidateOutput } from '../../../src/lib/types/workflow';
...
describe('validateWorkflow (CSV)', () => {
  ...
  describe('successful validation', () => {
    it('yields step events in correct order', async () => {
      ...
      const { events } = await runWorkflow({ filePath: testCsvPath });
    ...
    it('returns successful result with validation data', async () => {
      ...
      const { result } = await runWorkflow({ filePath: testCsvPath });
  ...
  describe('error handling', () => {
    it('fails gracefully when file does not exist', async () => {
      ...
      const { result, events } = await runWorkflow({ filePath: nonExistentPath });
    ...
    it('rejects XML files with clear error message', async () => {
      ...
      const { result } = await runWorkflow({ filePath: xmlPath });
  ...
  describe('step progression', () => {
    it('marks steps as complete in sequence', async () => {
      ...
      expect(completeEvents.map(e => e.step.id)).toEqual([
        'load',
        'parse',
        'validate',
        'report',
    ...
    it('stops on first error and does not proceed', async () => {
      ...
      const { result } = await runWorkflow({ filePath: nonExistentPath });
