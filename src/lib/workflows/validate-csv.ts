...
> {
  ...
  try {
    ...
    if (!input.filePath.toLowerCase().endsWith('.csv')) throw new Error(
    ...
    loadStep.error = error instanceof Error
      ? error
  ...
  try {
    ...
    parseStep.error = error instanceof Error
      ? error
      : new Error(String(error));
  ...
  try {
    validation = validateRows(csvData.rows, csvData.headers);
